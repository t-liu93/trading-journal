...
前端 F0（认证脚手架）和 F1（Account CRUD UI）已在 `refactoring/rebuild` 上交付。
后端 P6（stock/option/forex 三种 Instrument CRUD）刚刚完成（92 后端测试绿）。
后端 P7–P12 尚未启动；详见
...
| F-phase | 后端 gate | UI 交付 |
|---|---|---|
| **F2** | [P6](./backend-expansion-plan.zh.md#p6--instrument-base--extensions) ✅ + [P7](./backend-expansion-plan.zh.md#p7--strategyconfig) | `InstrumentPicker` 组件 + `/instruments` 浏览页 + `/settings/strategies` 页 |
| **F3** | [P8](./backend-expansion-plan.zh.md#p8--position) + [P10](./backend-expansion-plan.zh.md#p10--strategy-meta-extensions) + [P11](./backend-expansion-plan.zh.md#p11--tradeplan-event-stream) | Position 列表 + 创建/编辑 + 详情页（含 strategy-meta tabs 与 Plan tab） |
| **F4** | [P9](./backend-expansion-plan.zh.md#p9--trade) | Trade 录入（多腿 `order_group_id` UX）+ Position 详情 trade log |
| **F5** | [P12](./backend-expansion-plan.zh.md#p12--derived-read-layer) | Dashboards：分币种 PnL + 开仓/平仓表 + 图表 |
...
### F2 — Instrument 与 StrategyConfig 前端
