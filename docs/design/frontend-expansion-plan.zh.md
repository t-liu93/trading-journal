...
> 状态：**DRAFT v0.5**（2026-05-29）。F0（认证脚手架）和 F1（Account CRUD UI）
> 之后的前端扩展宏观路线图。与
...
F3（Position CRUD + 详情页）、F4（Trade 录入 UI）、F5（Dashboard）**均已**在
`refactoring/rebuild` 上交付（`vue-tsc` + `vite build` 全 clean；`schema.d.ts` 与
后端对得上）。**后端达到 V1 完整切片** —— P6 到 P12 全部出货（后端 406 条测试全绿；
`ruff` + `mypy --strict` clean）；详见
[backend-expansion-plan.zh.md §4](./backend-expansion-plan.zh.md#4-phase-roadmap)。
剩余唯一的 V1 phase 是 **F6**（单容器 Docker 生产接线）。
...
| **F3** | [P8](./backend-expansion-plan.zh.md#p8--position) ✅ + [P10](./backend-expansion-plan.zh.md#p10--strategy-meta-extensions) ✅ + [P11](./backend-expansion-plan.zh.md#p11--tradeplan-event-stream) ✅ + [P12.1](./backend-expansion-plan.zh.md#p12--derived-read-layer) ✅（列表上的 `net_cash_flow`） | Position 列表 + 创建/编辑 + 详情页（含 strategy-meta tabs 与 Plan tab） | ✅ 已完成（2026-05-28） |
| **F4** | [P9](./backend-expansion-plan.zh.md#p9--trade) ✅ | Trade 录入（多腿 `order_group_id` UX）+ Position 详情 trade log | ✅ 已完成（2026-05-28） |
| **F5** | [P12.2](./backend-expansion-plan.zh.md#p12--derived-read-layer) ✅（`/api/dashboard/summary`） | Dashboards：分币种 PnL + 开仓/平仓表 + 按月 PnL 图 | ✅ 已完成（2026-05-29） |
| **F6** | [后端 Phase 5](./mvp-implementation-plan.zh.md#phase-5--docker-single-container-deployment) | 单容器 Docker 生产构建接线（FastAPI 托管 `frontend/dist`） | ⏳ 下一步 |
...
### F3 — Position CRUD + 详情页（含 strategy-meta tabs + Plan tab）✅ 已完成（2026-05-28）

**详细 plan：** [frontend-implementation-plan-f3.md](./frontend-implementation-plan-f3.md)
...
### F4 — Trade 录入 ✅ 已完成（2026-05-28）

**详细 plan：** [frontend-implementation-plan-f4.md](./frontend-implementation-plan-f4.md)
...
### F5 — Dashboards 与图表 ✅ 已完成（2026-05-29）

**详细 plan：** [frontend-implementation-plan-f5.md](./frontend-implementation-plan-f5.md)
...
三项现已**全部敲定**（① 在 F2，② 在 F4，③ 在 F5）。下面内联记录结论。
...
1. **InstrumentPicker 在 option 场景下的 UX。** option 通过 5 元组
   ...
   的"选已有或创建"调用因 get-or-create 语义而免费。**F2 内敲定**（picker 先以
   select-only 出货；两步式创建在 F3 通过 `allowCreate` 落地）。
2. **Trade 录入多腿人体工学。** 两种设计：
   (a) 一个主"加 trade" modal 带"加一条 leg"按钮，行数生长，共享一个
   `order_group_id`；或
   (b) 预制流程（"开 iron condor"、"记 assignment"）自动发射正确的行。
   *倾向：* 上 (b) 并保留"自定义多腿"逃生口指向 (a)。**F4 内敲定（2026-05-28）：**
   交付通用 Custom multi-leg 表单（方案 (a)）作为主路径 —— `TradeEntryModal` +
   `TradeLegRow`，同一提交的各行共享 server 分配的 `order_group_id` —— 外加读侧
   pattern badge（`tradePatternBadge`）。Named flows (b) **未**实现，推迟到 V1.x。
3. **Dashboard 图表库。** ECharts（覆盖最广）、Plotly（数据科学熟悉）、Chart.js
   （最小）。*倾向：* **ECharts** — Vue 3 wrapper `vue-echarts` 成熟；最终想要
   的组合图区间（柱 + 线 + 分币种堆叠）覆盖良好。**F5 内敲定（2026-05-29）：**
   `MonthlyPnlChart` 交付，采用 `vue-echarts`（+ `echarts`）。
...
- **v0.5（2026-05-29）** — F3 + F4 + F5 全部交付（`vue-tsc` + `vite build` 全 clean；`schema.d.ts` 与后端对得上）。§1「当前状态」改写：F0–F5 全部完成，F6（Docker）是唯一剩余的 V1 phase。§3 映射表把 F3/F4/F5 行翻为 ✅ 已完成（F3/F4 2026-05-28，F5 2026-05-29），F6 翻为「⏳ 下一步」。§4 F3/F4/F5 标题标为已完成并链到各自 detail plan。§6 开放决策 ②（多腿人体工学）与 ③（图表库）收口：F4 交付 Custom multi-leg 表单为主路径（`TradeEntryModal`/`TradeLegRow` + pattern badge；named flows 推迟），F5 交付 `vue-echarts`。CI codegen gate 与 Vitest/Playwright 仍推迟（§5）—— F3–F5 未实现。
