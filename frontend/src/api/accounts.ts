 ...
 *
 * Thin typed wrapper around the backend's ``/accounts`` endpoints (Phase 4).
...
export const accountsApi = {
  ...
  list: (includeArchived = false) =>
    http.get(`/accounts?include_archived=${includeArchived}`) as Promise<Account[]>,
  ...
  update: (id: string, payload: AccountUpdate) =>
    http.patch(`/accounts/${id}`, payload) as Promise<Account>,
